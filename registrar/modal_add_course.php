...
$error_course=mysqli_query($connection,"select * from course where code='$code'")or die(mysqli_error($connection));
 $count=mysqli_num_rows($error_course);
 
 if ($count  != 1){
	mysqli_query($connection,"insert into course (code,title) values('$code','$title')")or die(mysqli_error($connection));
	}else{ ?>
